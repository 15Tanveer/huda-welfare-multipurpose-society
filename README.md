...
- `0004`–`0007` — category remap, the `resources` table and follow-ups.
- `0008_gallery_media.sql` — turns `gallery` into a mixed-media
  collection: adds `media_type` (`photo` | `video` | `press`, defaulting
  to `photo`), `video_url`, `video_source`, `source_name` and
  `coverage_url`, and makes `image_path` nullable so a video can be
  URL-only. It is additive: existing rows keep their data and simply
  become photos, so no backfill is needed.

Alternatively, paste the contents of the files, in order, into the
...
### Uploading photos, videos and press coverage
...
- **Gallery media** (shown on `/gallery`, filterable by program area
  *and* by media type): **Gallery → Add Media**. Pick a **Media Type**
  first — the rest of the form follows it:
  - **Photo** — upload the image, pick a category, optional title and
    caption.
  - **Video / Reel** — choose the source (YouTube, Instagram, Facebook or
    an external URL) and paste the normal watch/share link. A title is
    required; a cover image is optional (YouTube videos fall back to
    YouTube's own thumbnail). Only the link is stored — the player is
    generated by the site, never pasted-in embed code.
  - **Press & Media Coverage** — upload the clipping or screenshot, give
    it a title, and optionally record the publication (e.g. *Lokmat*) and
    a link to the original article.

  Every item can be linked to a program, edited later via **Gallery →
  Edit**, or deleted. Changing an item's media type (a photo that turns
  out to be a newspaper clipping, say) is an edit — nothing needs to be
...
exists), and the program's own page shows the full report followed by
**Photo Gallery**, **Program Videos** and **Media Coverage** — each
section appearing only when there is something to show. Those sections
combine the program's own photo set with any gallery item linked to the
program (the same photo added in both places is shown once).
